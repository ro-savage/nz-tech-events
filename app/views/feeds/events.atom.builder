...
xml.feed xmlns: "http://www.w3.org/2005/Atom", "xml:lang" => "en-NZ" do
  ...
  @events.each do |event|
    xml.entry do
      ...
      xml.category term: event.event_type, label: event.event_type.humanize

      if event.location_regions_display.present?
        xml.category term: event.location_regions_display, scheme: "#{root_url}regions"
      end
    end
  end
end
